...
//  int icons = (ConservativeReconstruction) ? 1 : 0;  // not implemented properly yet, TA
  ...
  if (CRModel > 1)
    v_cr1 = new float[GridDimension[0]];
  ...
  if (CRModel){
    ...
    if (CRModel > 1) {
      Prim1[10][i] = Prim[iCRFx][igrid];
      Prim1[11][i] = Prim[iCRFy][igrid];
      Prim1[12][i] = Prim[iCRFz][igrid];
    }
  }
