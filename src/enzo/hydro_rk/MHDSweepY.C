  ...
  */
{
  ...
  if (CRModel > 1)
    v_cr1 = new float[GridDimension[1]];
    ...
    for (i = 0; i < Xactivesize; i++) {
      ...
      for (j = 0; j < GridDimension[1]; j++) {
        ...
        if (CRModel){
          ...
          if (CRModel > 1) {
            Prim1[10][i] = Prim[iCRFy][igrid];
            Prim1[11][i] = Prim[iCRFz][igrid];
            Prim1[12][i] = Prim[iCRFx][igrid];
          }
        }
      }
